...
- 新增 `research/MODEL_LAG_ROOT_CAUSE_2026-09-14.md`，追查模型為何在高檔重複測壓與利率病灶復發時容易被市場追著跑。
- 結論：目前不是沒有觀念，而是「有觀念、缺主控序列」；慢性病灶、測壓、防呆、0/1 與主波段概念已存在，但尚未全部升級為盤前置頂預警。
- 改善方向鎖定：重複測壓計數器、病灶復發計數器、攻關前 2～5 次預定壓測序列、主波段置頂與低點/收回速度比較。
...
- `research/CORE_VALUES_FROM_DIALOGUE.md` 與 `research/HANDOFF_CURRENT.md` 補上鐵的記憶：主波段是複利核心，每日看盤是為了確認是否仍站在正確主波段上，而不是追逐每個小波動。
