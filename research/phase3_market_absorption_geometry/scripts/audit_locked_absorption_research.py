"""Independent integrity audit of the locked absorption confirmation."""
...
OUTPUT = WORKSPACE / "reports/market_absorption_geometry_v1_integrity.json"
HORIZON = 20
...
def main() -> None:
    ...
    snapshot["information_date"] = pd.to_datetime(
        snapshot["information_date"], errors="raise"
    )
    ...
    expected_target = {
        int(row["_twii_position"]): (1 if value > 0 else -1)
        for (_, row), value in zip(snapshot.iterrows(), future)
    ...
    checks = {
        ...
        "snapshot_hash": result["snapshot_sha256"]
        == config["hashes"][
            "research/phase3_market_absorption_geometry/data/locked_daily_feature_snapshot.csv"
        ],
        ...
        == config["data_semantics"]["snapshot_rows"],
    }
    for name, variant in result["variants"].items():
        ...
        checks[f"{name}_confirmation_dates"] = all(
            "2014-01-01" <= record["date"] <= "2025-12-31"
        ...
        checks[f"{name}_targets"] = all(
            expected_target.get(int(record["position"])) == int(record["target"])
            for record in records
        )
        checks[f"{name}_dates"] = all(
            expected_dates.get(int(record["position"])) == record["date"]
            for record in records
        )
        ...
        checks[f"{name}_baseline_hits"] = (
            base_hits == variant["baselines"][baseline]["hits"]
        )
        base_gate = (
        ...
        checks[f"{name}_base_gate"] = bool(base_gate) == bool(
            variant["passed_base_gate"]
        )

    left = {
        row["date"]: bool(row["hit"])
        for row in result["variants"]["price_control"]["records"]
    }
    right = {
        row["date"]: bool(row["hit"])
        for row in result["variants"]["plus_market_absorption_geometry"][
            "records"
        ]
    }
    ...
    increment = result["geometry_increment"]
    checks["paired_dates"] = len(common) == increment["identical_dates"]
    checks["paired_counts"] = (
        control_only == increment["control_only"]
        and treatment_only == increment["treatment_only"]
    )
    ...
    checks["increment_gate"] = bool(increment_passed) == bool(
        result["geometry_increment_passed"]
    )
    checks["control_final_gate"] = bool(
        result["variants"]["price_control"]["passed_base_gate"]
    ) == bool(result["variants"]["price_control"]["passed"])
    checks["treatment_final_gate"] = bool(
        result["variants"]["plus_market_absorption_geometry"]["passed_base_gate"]
        and increment_passed
    ) == bool(
        result["variants"]["plus_market_absorption_geometry"]["passed"]
    )
