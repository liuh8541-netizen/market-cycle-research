"""Independent integrity audit for nontraditional loan-collateral v1."""
...
RESULT = ROOT / "reports/loan_collateral_structure.json"
MANIFEST = ROOT / "data/processed/factors/loan_collateral_fetched_dates.csv"
FEATURES = ROOT / "data/processed/factors/loan_collateral_structure.csv"
OUTPUT = ROOT / "reports/loan_collateral_structure_integrity.json"
...
def main():
    ...
    right = {
        record["date"]: bool(record["hit"])
        for record in result["variants"]["plus_nontraditional_credit"]["records"]
    }
