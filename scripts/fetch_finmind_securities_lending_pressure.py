"""Incrementally cache point-in-time all-market securities-lending pressure."""
...
OUTPUT = ROOT / "data/processed/factors/securities_lending_pressure.csv"
MANIFEST = ROOT / "data/processed/factors/securities_lending_fetched_dates.csv"
LOCK = ROOT / "data/processed/factors/.securities_lending_fetch.lock"
TRADING_DATES = (
...
def aggregate(day, rows):
    ...
    if data.empty or "stock_id" not in data:
        return {
            "date": day, "lending_stock_count": 0, "lending_trade_count": 0,
            "lending_volume": 0.0, "lending_money": 0.0,
            "lending_weighted_fee_rate": np.nan,
            "lending_median_fee_rate": np.nan,
            "lending_volume_top10_share": np.nan,
            "lending_competitive_fraction": np.nan,
        }
    ids = data["stock_id"].astype(str).str.strip()
    data = data.loc[ids.map(lambda value: bool(COMMON_STOCK.fullmatch(value)))].copy()
    volume = pd.to_numeric(data.get("volume"), errors="coerce").fillna(0).clip(lower=0)
    close = pd.to_numeric(data.get("close"), errors="coerce").fillna(0).clip(lower=0)
    fee = pd.to_numeric(data.get("fee_rate"), errors="coerce")
    valid_fee = fee.notna() & volume.gt(0)
    stock_volume = volume.groupby(data["stock_id"].astype(str)).sum()
    total_volume = float(volume.sum())
    transaction = data.get("transaction_type", pd.Series("", index=data.index)).astype(str)
    return {
        "date": day,
        "lending_stock_count": int(data["stock_id"].nunique()),
        "lending_trade_count": int(len(data)),
        "lending_volume": total_volume,
        "lending_money": float((volume * close).sum()),
        "lending_weighted_fee_rate": (
            float(np.average(fee[valid_fee], weights=volume[valid_fee]))
            if valid_fee.any() else np.nan
        ),
        "lending_median_fee_rate": float(fee.median()) if fee.notna().any() else np.nan,
        "lending_volume_top10_share": (
            float(stock_volume.nlargest(10).sum() / total_volume)
            if total_volume > 0 else np.nan
        ),
        "lending_competitive_fraction": float(
            transaction.str.contains("競價", regex=False).mean()
        ),
    }
...
def main():
    ...
    dates = sorted(
        source.loc[
            ...
            & source["date"].between(args.start_date, args.end_date),
            "date",
    ...
    LOCK.write_text(
        json.dumps({"pending": len(pending), "total": len(dates)}), encoding="utf-8"
    )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if done % args.checkpoint == 0:
                    atomic_csv(
                        pd.DataFrame(features).sort_values("date").drop_duplicates("date", keep="last"),
                        OUTPUT,
                    )
                    atomic_csv(
