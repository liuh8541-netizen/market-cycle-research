"""Wait for the order-book backfill, audit it, then run the locked experiment once."""
...
FEATURES = ROOT / "data/processed/factors/orderbook_early_pulse.csv"
CONFIG = ROOT / "config/locked_orderbook_early_pulse_close_v1.json"
AUDIT = ROOT / "reports/orderbook_early_pulse_integrity.json"
RESULT = ROOT / "reports/orderbook_early_pulse_close.json"
RUN_LOG = ROOT / "reports/orderbook_early_pulse_formal_run.log"
...
def audit():
    ...
    checks = {
        ...
        "observation_min": int(numeric.observations.min()),
        "observation_max": int(numeric.observations.max()),
    ...
    passed = (
        ...
        and checks["observation_min"] == 181
        and checks["observation_max"] == 181
...
def main():
    ...
    if not audit():
        raise RuntimeError(f"Order-book integrity audit failed; see {AUDIT}")
    if RESULT.exists():
        raise RuntimeError("Formal result already exists; refusing to execute twice")
    process = subprocess.run(
        [sys.executable, "-u", str(ROOT / "scripts/research_orderbook_early_pulse_close.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
