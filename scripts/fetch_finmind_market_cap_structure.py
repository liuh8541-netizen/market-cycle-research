"""Incrementally cache point-in-time all-market capitalization structure."""
...
OUTPUT = ROOT / "data/processed/factors/market_cap_structure.csv"
MANIFEST = ROOT / "data/processed/factors/market_cap_fetched_dates.csv"
LOCK = ROOT / "data/processed/factors/.market_cap_fetch.lock"
...
def aggregate(day, rows):
    ...
    if data.empty or "stock_id" not in data:
        return {"date": day, "market_cap_stock_count": 0}
    ids = data["stock_id"].astype(str).str.strip()
    values = pd.to_numeric(data["market_value"], errors="coerce")
    valid = (
        ids.map(lambda value: bool(COMMON_STOCK.fullmatch(value)))
        & values.gt(0) & np.isfinite(values)
    )
    values = values[valid].groupby(ids[valid]).sum().sort_values(ascending=False)
    total = float(values.sum())
    shares = values / total if total > 0 else values * np.nan
    logs = np.log(values)
    return {
        "date": day,
        "market_cap_stock_count": int(len(values)),
        "market_cap_total": total,
        "market_cap_top1_share": float(shares.head(1).sum()),
        "market_cap_top5_share": float(shares.head(5).sum()),
        "market_cap_top10_share": float(shares.head(10).sum()),
        "market_cap_top20_share": float(shares.head(20).sum()),
        "market_cap_top50_share": float(shares.head(50).sum()),
        "market_cap_bottom_half_share": float(shares.tail(len(shares) // 2).sum()),
        "market_cap_hhi": float((shares ** 2).sum()),
        "market_cap_log_median": float(logs.median()),
        "market_cap_log_iqr": float(logs.quantile(0.75) - logs.quantile(0.25)),
        "market_cap_log_q90_q10": float(logs.quantile(0.90) - logs.quantile(0.10)),
    }
